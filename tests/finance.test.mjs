...
test('monthly financial trends preserve missing months, negative margins, and the selling split',async()=>{
 ...
 assert.equal(rows[0].grossMargin,-.2);assert.equal(rows[0].operatingMargin,-.7);assert.equal(rows[0].netMargin,-.8);assert.equal(rows[0].operatingBudget,30);assert.equal(rows[0].sellingExpenses,20);assert.equal(rows[0].totalOpex,50);
 assert.equal(rows[1].grossMargin,null);assert.equal(rows[1].operatingMargin,null);assert.equal(rows[1].netMargin,null);assert.deepEqual(rows[2],{month:'2026-03'});
});
