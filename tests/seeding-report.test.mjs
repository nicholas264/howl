...
import { reportMonths, summarizeMonth } from '../src/lib/seeding-report.js';
...
test('monthly report separates dated costs, excludes future costs and deduplicates creator ad links', async () => {
  ...
  try {
    ...
    await db.exec(`CREATE TABLE creator_seeding_log (seeded_on date, unit_cogs numeric, quantity integer, shipping_cost numeric, creator_fee numeric);
      CREATE TABLE creative_insights_daily (ad_id text, date date, spend numeric, purchase_value numeric);
      CREATE TABLE launch_history (ad_id text, creator_id integer, creator text);
      CREATE TABLE creators (id integer, name text);
      CREATE TABLE creative_performance (ad_id text, group_key text);
      CREATE TABLE creative_creator_assignments (group_key text, creator_id integer);
      CREATE TABLE creative_assets (group_key text, creator_id integer);
      INSERT INTO creator_seeding_log VALUES ('2020-02-01',100,2,20,50), ('2020-01-01',1,1,2,3), (NULL,10,1,0,5), ('2999-01-01',400,1,0,600);
      INSERT INTO creative_insights_daily VALUES ('linked','2020-02-01',100,1000), ('unlinked','2020-02-01',5000,50000);
      INSERT INTO launch_history VALUES ('linked',1,NULL),('linked',2,NULL);
      INSERT INTO creative_performance VALUES ('linked','g');
      INSERT INTO creative_creator_assignments VALUES ('g',1),('g',2);
