...
export function createQuickBooksCallback({getSql=()=>neon(process.env.DATABASE_URL),env=process.env,fetcher=fetch}={}){return async(req,res)=>{
 ...
 const sql=getSql();const [s]=await sql`DELETE FROM finance_oauth WHERE state=${hash(state)} AND binding=${hash(binding)} AND expires_at>now() RETURNING user_id`;
 ...
 const t=await exchange({grant_type:'authorization_code',code,redirect_uri:env.QUICKBOOKS_REDIRECT_URI},env,fetcher),tokens=encrypt(t,env),expires=new Date(Date.now()+t.expires_in*1000).toISOString(),version=nonce();
 // Recheck owner at commit. Reconnection clears cached data and account mapping.
 const results=await sql.transaction([
 sql`INSERT INTO finance_connection(id,realm,tokens,expires_at,version,environment,connected_by) SELECT 'company',${realmId},${tokens},${expires},${version},${env.QUICKBOOKS_ENVIRONMENT},${s.user_id} FROM app_users WHERE user_id=${s.user_id} AND role='owner' AND status='active' ON CONFLICT(id) DO UPDATE SET realm=EXCLUDED.realm,tokens=EXCLUDED.tokens,expires_at=EXCLUDED.expires_at,version=EXCLUDED.version,environment=EXCLUDED.environment,connected_by=EXCLUDED.connected_by,lease_until=NULL RETURNING id`,
 sql`UPDATE finance_workspace SET snapshot=NULL,settings=jsonb_set(settings,'{mapping}','{}'::jsonb),revision=revision+1 WHERE id='company' AND EXISTS(SELECT 1 FROM finance_connection WHERE version=${version})`
