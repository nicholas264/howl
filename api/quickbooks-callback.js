...
export function createQuickBooksCallback({getSql=()=>neon(process.env.DATABASE_URL),env=process.env,fetcher=fetch}={}){return async(req,res)=>{
 ...
 sql`WITH consent AS (DELETE FROM finance_oauth WHERE state=${hash(state)} AND binding=${hash(binding)} AND claimed_at IS NOT NULL AND expires_at>now() RETURNING user_id) INSERT INTO finance_connection(id,realm,tokens,expires_at,version,environment,connected_by) SELECT 'company',${realmId},${tokens},${expires},${version},${env.QUICKBOOKS_ENVIRONMENT},${s.user_id} FROM app_users JOIN consent ON consent.user_id=app_users.user_id WHERE app_users.user_id=${s.user_id} AND role='owner' AND status='active' ON CONFLICT(id) DO UPDATE SET realm=EXCLUDED.realm,tokens=EXCLUDED.tokens,expires_at=EXCLUDED.expires_at,version=EXCLUDED.version,environment=EXCLUDED.environment,connected_by=EXCLUDED.connected_by,lease_until=NULL RETURNING id`,
