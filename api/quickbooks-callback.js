...
export function createQuickBooksCallback({getSql=()=>neon(process.env.DATABASE_URL),env=process.env,fetcher=fetch}={}){return async(req,res)=>{
 ...
 sql`UPDATE finance_workspace SET snapshot=NULL,settings=jsonb_set(settings-'ebitdaAdjustments'-'sellingAccountIds'-'productItemMapping'-'productCogsAccounts'-'productRevenueMapping','{mapping}','{}'::jsonb),revision=revision+1 WHERE id='company' AND EXISTS(SELECT 1 FROM finance_connection WHERE version=${version})`
