...
export default async function handler(req,res) {
  ...
  try{
    ...
    if(!input || typeof input!=='object' || !Array.isArray(media) || !media.length || media.length>2)
      return res.status(400).json({error:'One creative with one or two media references is required.'});
    if(new Set(media.map(item=>item?.role)).size!==media.length || media.some(item=>!item || !['single','feed','story'].includes(item.role)))return res.status(400).json({error:'Unique media placement roles are required.'});
    ...
    for(const item of media){
      ...
      if(item.drive_file_id){
        ...
        const result=await fetchPublicResource(readable,{maxBytes:2*1024*1024*1024,timeoutMs:45000,hashOnly:true,contentTypes:/^(video|image)\//i});
