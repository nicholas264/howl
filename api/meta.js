    ...
    switch (action) {
      ...
      case 'analyze_creative_group': {
        ...
        const [topAd] = await sql`
          ...
          LIMIT 1
        `;
        if (!topAd) return res.status(404).json({ error: 'No ads found for group' });
...
Analyze this creative.`;
        ...
        const claudeContent = [];
        if (imageB64) {
          claudeContent.push({ type: 'image', source: { type: 'base64', media_type: mediaType, data: imageB64 } });
        }
        ...
        if (claudeData.error) return res.status(400).json({ error: claudeData.error.message, step: 'claude' });
        ...
        try {
          ...
          return res.status(500).json({ error: 'Failed to parse analysis JSON', raw: text });
        }
      ...
      case 'list_analyzed_winners': {
        ...
        const rows = await sql`
          ...
          LIMIT 100
        `;
        return res.json({ winners: rows, sinceDays });
      }
