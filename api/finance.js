...
export function createFinanceHandler({authorize=requirePermission,env=process.env,fetcher=fetch,now=()=>new Date()}={}){return async(req,res)=>{
 ...
 await sql.transaction([sql`DELETE FROM finance_connection WHERE id='company'`,sql`UPDATE finance_workspace SET snapshot=NULL,settings=jsonb_set(settings-'ebitdaAdjustments'-'sellingAccountIds'-'productItemMapping'-'productCogsAccounts'-'productRevenueMapping','{mapping}','{}'::jsonb),revision=revision+1 WHERE id='company'`,sql`DELETE FROM finance_oauth WHERE user_id=${userId}`]);return res.json({ok:true});
 }
 if(b.action==='save'){
