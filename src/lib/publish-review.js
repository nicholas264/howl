import {creativeTestIntent} from './creative-test-intent.js';
...
export async function preparePublishReview(item,config,adsetId,{fetchImpl,signal,creativeTest}) {
  if((!creativeTest&&(!adsetId || adsetId==='__new__')) || !config.pageId?.trim() || !config.destUrl?.trim() || !item.hook?.trim())throw new Error('Choose an ad set, Page, destination and headline before reviewing.');
  if(item.storyUrl)throw new Error('Use the Launcher for paired feed and story assets.');
  const intent=creativeTest?creativeTestIntent(creativeTest):null;
  ...
  const response=await fetchImpl('/api/launch-review',{method:'POST',headers:{'Content-Type':'application/json'},signal,
    body:JSON.stringify({input:{action:'create_ad_from_creative',...publishAttribution(item)},media,...(intent?{creative_test:creativeTest}:{adset_id:adsetId})})});
  const result=await response.json();if(!response.ok || result.error)throw new Error(result.error || 'Launch review failed.');
  return {item,approvals:result.approvals,plan:{version:1,confirmed:false,ad_name:item.name || (intent?'Untitled':`HOWL Ad ${new Date().toLocaleDateString()}`),
    ...
    target:intent?{mode:'creative_test',campaign:result.creative_test.campaign,request:{...result.creative_test.adset,name:item.name || 'Untitled'}}:{mode:'existing',id:adsetId,snapshot:result.adset},
