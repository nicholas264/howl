    ...
    <div style={S.wrap}>
      ...
      {(() => {
        ...
        const rows = recent13.map(mk => {
          ...
          const ncac = sh.newCustomers > 0 ? metaSpend / sh.newCustomers : null;
          const blendedNcac = sh.newCustomers > 0 ? adSpend / sh.newCustomers : null;
          ...
          return { month: mk, revenue, orders, newCustomers: sh.newCustomers || 0, returningCustomers: sh.returningCustomers || 0, newRevenue: sh.newRevenue || 0, returningRevenue: sh.returningRevenue || 0, metaSpend, googleSpend, adSpend, cogs, cogsActualPct, paymentFees, shipCost, fulfill, cm3, ncac, blendedNcac, blendedRoas, newRoas, firstOrderPayback, opex: opexThis, opexCoverage, isCurrent };
        });
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '90px 1fr 1fr', gap: 6, alignItems: 'center', maxHeight: 360, overflowY: 'auto' }}>
                    ...
                    {recent13.map(mk => (
                      <React.Fragment key={mk}>
                        ...
                        <input
                          type="number" step="1" placeholder={String(defaultOpex)}
                          value={settings.opexByMonth?.[mk] ?? ''}
                          onChange={e => setSettings({ ...settings, opexByMonth: { ...(settings.opexByMonth || {}), [mk]: e.target.value === '' ? undefined : parseFloat(e.target.value) || 0 } })}
