...
const TEXT_COLORS = [
  ...
  { id: 'dark',   label: 'Dark',   value: '#f4f1ea' },
    ...
    <>
      <div>
        <div style={{ ...S.label, display: 'flex', justifyContent: 'space-between' }}>
          <span>Headline Size</span><span style={{ color: '#171717' }}>{fontSize}px</span>
        </div>
        <input type="range" min={32} max={200} step={4} value={fontSize} onChange={e => setFontSize(Number(e.target.value))} style={{ width: '100%', accentColor: '#d84a17' }} />
      </div>

      <div>
        <div style={{ ...S.label, display: 'flex', justifyContent: 'space-between' }}>
          <span>Body Size</span><span style={{ color: '#171717' }}>{bodyFontSize}px</span>
        </div>
        <input type="range" min={14} max={120} step={2} value={bodyFontSize} onChange={e => setBodyFontSize(Number(e.target.value))} style={{ width: '100%', accentColor: '#d84a17' }} />
      </div>
          ...
          {TEXT_COLORS.map(c => {
            ...
            return (
              <button key={c.id} onClick={() => setColorId(c.id)} style={{
                ...
                border: `2px solid ${colorId === c.id ? '#d84a17' : '#dedbd3'}`,
        ...
        {showPresetInput && (
          <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
            <input
              ...
              style={{ flex: 1, padding: '5px 8px', border: '1px solid #dedbd3', borderRadius: 4, fontFamily: 'inherit', fontSize: 10, outline: 'none' }}
            />
            <button onClick={savePreset} style={{ ...S.link, color: '#fff', background: '#d84a17', padding: '5px 10px', borderRadius: 4 }}>Save</button>
          </div>
        )}
        {presets.length > 0 && (
            ...
            {presets.map(p => (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <button onClick={() => applyPreset(p)} style={{
                  ...
                  border: '1px solid #dedbd3', background: '#f4f1ea', color: '#171717',
                  fontFamily: 'inherit', fontSize: 10,
                }}>{p.name} <span style={{ color: '#b0a898', fontSize: 9 }}>{p.fontSize}px · {p.colorId}</span></button>
                <button onClick={() => deletePreset(p.id)} style={{ ...S.link, color: '#c9c4ba', fontSize: 12 }}>×</button>
              </div>
            ))}
          </div>
        )}
