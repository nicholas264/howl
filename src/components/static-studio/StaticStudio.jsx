...
import './studio.css';
import StudioCosts from './StudioCosts.jsx';
import { finishStudioBatch } from '../../lib/static-studio/batch.js';
...
const invalidate=c=>{const {render,review,approval,launchedToQueue,batchError,...draft}=c;return draft;};
...
export default function StaticStudio({onAddToCart,onOpenLauncher,driveAuth}) {
  ...
  const [zoom,setZoom]=useState(false),[driveConnected,setDriveConnected]=useState(false);
  ...
  const changeAsset=(id,patch)=>update(current=>({...current,assets:current.assets.map(a=>a.id===id?{...a,...patch}:a),concepts:current.concepts.map(c=>c.assetId===id && Object.keys(patch).some(k=>['productId','approved','role','features','protectedRegion','url','sha256'].includes(k))?invalidate(c):c)}));
  ...
  async function analyzeAsset(target=asset) {
    await flush();const data=await studioRequest({action:'analyze',assetId:target.id});
    changeAsset(target.id,{analysis:[data.description,...data.uncertainties.map(s=>`Check: ${s}`)].join('\n'),assessment:normalizeAssessment(data),...(!target.protectedRegion && data.suggestedProtectedRegion?{protectedRegion:data.suggestedProtectedRegion}:{})});await flush();
  ...
  async function finishConcept(c) {
    if(c.approval || c.review?.verdict==='pass')return c;
    let next=c.review?.verdict==='revise'?c:await reviewConcept(c);
    for(let attempt=0;attempt<2 && next.review.verdict==='revise' && !cancelRef.current;attempt++) {
      setBusy(`Art director is revising the concept (${attempt+1}/2)`);
  ...
  async function finishBatch(concepts=visibleConcepts) {
    const result=await finishStudioBatch({ids:concepts.map(c=>c.id),
      latest:async id=>(await flush()).concepts.find(c=>c.id===id),cancelled:()=>cancelRef.current,
      onProgress:(i,total,c)=>{setConceptId(c.id);setBusy(`Finishing concept ${i} of ${total}`);},
      finish:async c=>{
        try {const next=await finishConcept(c);delete next.batchError;writeConcept(next);await flush();return next;}
        catch(err){const current=(await flush()).concepts.find(row=>row.id===c.id);writeConcept({...current,batchError:err.message});await flush();throw err;}
      }});
  ...
  return <div className="ss" aria-busy={!!busy}>
    ...
    {view==='library' && <div className="ss-library-layout">
      ...
      <aside className="ss-inspector">{asset?<><h2>{asset.role==='reference'?'Reference notes':'Photograph details'}</h2><div className={`ss-anchor-photo ${anchorName || protecting?'is-placing':''}`} onClick={e=>{
        ...
        <button className="ss-button ss-full" onClick={()=>setZoom(true)}>Inspect full photograph</button><p className="ss-filename">{asset.name}</p><div className="ss-readiness"><strong>{assetReadiness(asset).label}</strong>{asset.assessment?.suggestedProductId && <p>Suggested model: {productFor(asset.assessment.suggestedProductId).name}. Compare against the original before confirming.</p>}{asset.assessment?.uncertainties.length>0 && <ul>{asset.assessment.uncertainties.map((issue,i)=><li key={i}>{issue}</li>)}</ul>}</div>{w.concepts.some(c=>c.assetId===asset.id) && <p>Used in saved concepts. Remove those concepts before changing its identity.</p>}<label>Use as<select disabled={w.concepts.some(c=>c.assetId===asset.id)} value={asset.role} onChange={e=>changeAsset(asset.id,{role:e.target.value,approved:false,features:[],protectedRegion:null})}><option value="product">Product photograph</option><option value="reference">Design reference</option></select></label>
    ...
    {view==='create' && <div className="ss-create"><section><h2>Give the batch a point of view.</h2><p className="ss-intro">A good batch explores ideas, not just permutations. Choose your products and give the art director a brief.</p><label>Creative brief<textarea rows="5" value={w.brief} onChange={e=>update({...w,brief:e.target.value})} placeholder="For example: Fall camping. Lead with the moment around the fire. Keep copy short and make each product unmistakable."/></label><div className="ss-product-choices">{PRODUCTS.map(p=><label key={p.id} className={w.selectedProducts.includes(p.id)?'is-selected':''}><input type="checkbox" checked={w.selectedProducts.includes(p.id)} onChange={e=>update({...w,selectedProducts:e.target.checked?[...w.selectedProducts,p.id]:w.selectedProducts.filter(id=>id!==p.id)})}/><strong>{p.name}</strong><small>{eligible.filter(a=>a.productId===p.id).length} confirmed originals</small></label>)}</div><div className="ss-count"><label>Concepts per product<select value={w.count} onChange={e=>update({...w,count:Number(e.target.value)})}>{[1,2,3,4,5].map(n=><option key={n} value={n}>{n} concept{n===1?'':'s'}</option>)}</select></label><p><strong>{(w.selectedProducts.length-missing.length)*w.count*2} exports</strong><br/>4:5 feed + 9:16 story for every concept</p></div>
      <h3>Image-led art direction</h3><p>Full-bleed photography. Text directly on the scene. Each placement is composed around the protected product.</p><details><summary>Legacy manual layout styles</summary><div className="ss-directions"><button className={w.direction==='mix'?'is-selected':''} onClick={()=>update({...w,direction:'mix'})}><div className="ss-swatch ss-swatch-mix"/><strong>Curated mix</strong><span>Let the art director match each image.</span></button>{DIRECTIONS.map(d=><button key={d.id} className={w.direction===d.id?'is-selected':''} onClick={()=>update({...w,direction:d.id})}><div className="ss-swatch" style={{background:d.background,color:d.foreground}}><b>HOWL</b><i/></div><strong>{d.name}</strong><span>{d.description}</span></button>)}</div></details>
      ...
      <div className="ss-generate-actions"><button className="ss-button ss-primary" disabled={!w.selectedProducts.length || missing.length===w.selectedProducts.length || w.concepts.length>=120} onClick={()=>run('Creating finished batch',()=>generate(true,true))}>Create finished batch</button><button className="ss-button" disabled={!w.selectedProducts.length || missing.length===w.selectedProducts.length || w.concepts.length>=120} onClick={()=>run('Composing the batch',()=>generate(false))}>Manual layout samples</button></div><p className="ss-caption">AI ads use edge-to-edge photographs with text directly on the image. Confirm each product’s protected region first. The art director writes fresh hooks from your photos and brief, and cites verified product facts. The batch automatically renders, reviews, and refines both placements. Keep this tab open while it runs. Manual layouts are legacy starting points.</p>
    ...
    {view==='review' && <>
      ...
      {concept && source && <div className="ss-review-layout"><section className="ss-workbench"><div className="ss-workbench-header"><div><h3>{concept.angle}</h3>{concept.premise && <p>{concept.premise}</p>}{concept.visualIdea && <p>{concept.visualIdea}</p>}<p>{concept.rationale}</p></div><label className="ss-check"><input type="checkbox" checked={safeAreas} onChange={e=>setSafeAreas(e.target.checked)}/>Show safe areas</label></div>
        ...
        {concept.batchError && <div className="ss-alert" role="alert">{concept.batchError}</div>}{concept.composition && <p className="ss-caption">Art-directed composition: separate photo and type placement for each format.</p>}{concept.review && <div className={`ss-visual-review ${concept.review.verdict}`}><h4>{concept.review.verdict==='pass'?'Visual review passed':'Revisions recommended'}</h4><p>{concept.review.summary}</p>{concept.review.issues.length>0 && <ul>{concept.review.issues.map((issue,i)=><li key={i}>{issue}</li>)}</ul>}</div>}
