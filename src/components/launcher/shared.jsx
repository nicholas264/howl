...
export function ProfileAvatar({ src, name, size = 24 }) {
  ...
  return (
    <div style={{
      ...
      background: 'var(--ui-border, #dedbd3)', color: 'var(--ui-ink, #171717)',
    ...
    <div style={{ position: 'relative' }}>
      ...
      >
        {selected ? (
          ...
          <span style={{ flex: 1, color: 'var(--ui-subtle, #88857f)' }}>
            {loading ? 'Loading Pages…' : error ? `Error: ${error}` : pages.length ? 'Select Facebook Page' : 'No Pages found'}
          </span>
        )}
        <span style={{ color: 'var(--ui-subtle, #88857f)', fontSize: 9 }}>▼</span>
      </button>
      {open && (
        <>
          ...
          <div style={{
            ...
            background: '#fff', border: '1px solid var(--ui-border, #dedbd3)', borderRadius: 4,
            maxHeight: 280, overflowY: 'auto', boxShadow: '0 8px 24px rgba(0,0,0,0.5)',
          }}>
            {pages.length === 0 && (
              <div style={{ padding: 12, color: 'var(--ui-muted, #77746f)', fontSize: 11 }}>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    border: 0, color: 'var(--ui-ink, #171717)', fontFamily: 'inherit', fontSize: 11,
                  }}
                >
                  <ProfileAvatar src={p.picture_url} name={p.name} size={28} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600 }}>{p.name}</div>
                    <div style={{ fontSize: 9, color: 'var(--ui-subtle, #88857f)' }}>{p.username ? `@${p.username} · ` : ''}{p.id}</div>
                  </div>
                </button>
              );
            })}
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 9, color: 'var(--ui-subtle, #88857f)' }}>{a.id} · {a.source}</div>
                  </div>
                </button>
              );
            })}
