  ...
  return <>
    <section className="cd-recommendation" aria-labelledby="recommendation-title">
      ...
      {recommendedMonth && recommendation.ready ? <>
        ...
        <p className="cd-format-note">Total across video and static ads. A format split has not been set.</p>
        ...
        <details className="cd-recommendation-evidence"><summary>Why this number?</summary><p>{money(recommendedMonth.metaBudget)} Meta limit × {assumptions.testBudgetPct}% unsuccessful-test reserve = {money(recommendedMonth.reserve)}. Historical unsuccessful-test spend averaged {money(lifecycle.curves[0].unsuccessfulPerLaunch)} per launch, including the launches that became winners in the denominator. That reserve sets a starting ceiling of {lifecycle.curves[0].unsuccessfulPerLaunch > 0 ? Math.floor(recommendedMonth.reserve / lifecycle.curves[0].unsuccessfulPerLaunch) : 'an undetermined number of'} new ads; timing, future-month budgets, and team capacity can reduce it.</p><p>The historical sample had {lifecycle.curves[0].winners} first-month winners among {lifecycle.curves[0].sample} mature assets. A winner means at least {money(assumptions.minWinnerSpend)} spend, {assumptions.minPurchases} purchases, and {assumptions.roasFloor}× Meta ROAS in 30 days. This is separate from the 4.5× aMER target. The pooled model does not determine an optimal video/static mix.</p></details>
