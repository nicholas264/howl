...
export default function LifecyclePlanning({history, assumptions}) {
  const [selectedAmer, setSelectedAmer] = useState(4.55);
  ...
  const scenarios = useMemo(() => dateValid ? [4, 4.55, 5].map(a => planFundedCreative(history, lifecycle, assumptions, a, planningDate)) : [], [history, lifecycle, assumptions, planningDate, dateValid]);
