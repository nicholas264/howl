  ...
  return <>
    <section className="cd-recommendation" aria-labelledby="recommendation-title">
      ...
      {recommendedMonth && recommendation.ready ? <>
        <h2 id="recommendation-title">{recommendedMonth.launches > 0 ? <>Produce {recommendedMonth.launches} new ads for {new Date(recommendedMonth.month + '-01T12:00:00Z').toLocaleDateString('en-US', {month:'long', timeZone:'UTC'})}.</> : 'Rework the testing budget before adding new ads.'}</h2>
        <p className="cd-recommendation-lead">{launchBatches.length ? `Launch in ${launchBatches.length} weekly batches, starting ${dateLabel(launchBatches[0].launchDate)}.` : 'Earlier planned tests use the available reserve. Open the budget detail to adjust the plan.'}</p>
        {launchBatches.length > 0 && <ol className="cd-launch-batches" aria-label="Recommended launches">{launchBatches.map(b => <li key={b.launchDate}><strong>{b.assets} ads</strong><span>Launch {dateLabel(b.launchDate)}</span><small>Brief by {dateLabel(b.briefBy)}</small></li>)}</ol>}
