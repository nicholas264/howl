...
function Budget({ label, used, budget, tone }) {
  ...
  return <div className={`seed-budget-meter ${tone}`}>
    <h3>{label}</h3>
    <div className="seed-budget-numbers"><div><strong>{money(used)}</strong><span>{tone === 'creator' ? 'used + reserved' : 'used'}</span></div><div><strong className={remaining < 0 ? 'over' : ''}>{remaining === null ? '—' : money(Math.abs(remaining))}</strong><span>{remaining === null ? 'budget not set' : remaining < 0 ? 'over budget' : 'left'}</span></div></div>
...
export default function SeedingBudgetDashboard({ report, month, onMonth, canManage, onSaved }) {
  ...
  const max = Math.max(1, ...chart.map(m => Math.max(m.budgetUsed, m.budget ? Number(m.budget.seeding) + Number(m.budget.creator) : 0)));
  ...
  return <section className="seed-budget-dashboard" aria-label="Seeding and creator budgets">
    ...
    <div className="seed-budget-pair"><Budget label="Seeding · product + shipping" used={metrics.seeding} budget={metrics.budget?.seeding} tone="seeding" /><Budget label="Creator fees" used={metrics.creatorBudgetUsed} budget={metrics.budget?.creator} tone="creator" /></div>
    <p className="seed-budget-note">Creator fees reserve existing approved, active, and completed USD agreements for the month, including monthly retainers. Linked ledger fees count once. Undated agreements use their approval or creation date.{metrics.scheduled > 0 && ` ${money(metrics.scheduled)} in future ledger costs is excluded; agreement commitments are still reserved.`}{report.monthly.some(r => r.month === 'undated') && ' Undated entries are excluded; choose Undated in the ledger to review them.'}</p>
    ...
    <div className="seed-budget-chart-scroll"><div className="seed-budget-scale"><span>{max === 1 ? 'No recorded investment' : money(max)}</span><span>USD · recorded costs + fee commitments</span></div><div className="seed-budget-chart" style={{ minWidth: `${Math.max(600, chart.length * 92)}px` }}>
      {chart.map(item => <button type="button" className={`seed-budget-column ${item.month === selected ? 'selected' : ''}`} key={item.month} aria-pressed={item.month === selected} onClick={() => select(item.month)} disabled={saving} aria-label={`${monthLabel(item.month)}: seeding ${money(item.seeding)}, creator fees ${money(item.creatorBudgetUsed)}, combined budget ${item.budget ? money(Number(item.budget.seeding) + Number(item.budget.creator)) : 'not set'}`}>
        <span className="seed-budget-bars" title={`Seeding ${money(item.seeding)} · Creator fees ${money(item.creatorBudgetUsed)}`}><span className="seed-budget-stack" style={{ height: `${item.budgetUsed / max * 100}%` }}><i className="creator" style={{ flex: Number(item.creatorBudgetUsed) }} /><i className="seeding" style={{ flex: Number(item.seeding) }} /></span>{item.budget && <i className="seed-budget-target" style={{ bottom: `${(Number(item.budget.seeding) + Number(item.budget.creator)) / max * 100}%` }} />}</span>
        <strong>{money(item.budgetUsed)}</strong><span>{monthLabel(item.month)}</span>
      </button>)}
    </div></div>
