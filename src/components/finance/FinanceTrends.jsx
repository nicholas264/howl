...
export default function FinanceTrends({ periods, months, snapshot, settings, money, breakEven }) {
 ...
 const metrics = [
  ...
  ['Total OpEx / revenue', percent(ratio(expenses, revenue)), 'YTD total operating expenses, including selling'],
 ...
 return <>
  <div className="fin-trends-grid">
   <TrendChart title="Monthly margin trends" note="Gross margin is after COGS; contribution margin is after COGS and the selected selling expenses; net margin is after all reported expenses and other income. OpEx excluding selling / revenue measures operating expenses excluding the selected selling expenses, matching the break-even operating budget. COGS are excluded. Each percentage uses that month’s revenue; zero or negative revenue is shown as unavailable." rows={rows} series={[{ key: 'grossMargin', label: 'Gross margin', color: '#635bce' }, { key: 'contributionMargin', label: 'Contribution margin', color: '#168477' }, { key: 'netMargin', label: 'Net margin', color: '#bc743a' }, { key: 'opexRatio', label: 'OpEx excluding selling / revenue', color: '#347fa3' }]} format={percent}/>
   <TrendChart title="Operating expenses over time" note={sellingModel ? 'Operating expenses excluding selling and the selected selling expenses are shown separately. Together they equal total operating expenses; COGS and other expenses are excluded.' : 'Total operating expenses from QuickBooks, including selling expenses. Excludes COGS and other expenses.'} rows={rows} series={sellingModel ? [{ key: 'operatingBudget', label: 'OpEx excluding selling', color: '#635bce' }, { key: 'sellingExpenses', label: 'Selling expenses', color: '#168477' }] : [{ key: 'totalOpex', label: 'Total OpEx', color: '#635bce' }]} format={(value, compact) => compact ? new Intl.NumberFormat('en', { notation: 'compact', maximumFractionDigits: 1 }).format(value) : money(value)}/>
  </div>
