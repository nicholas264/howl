...
export default function FinanceTrends({ periods, months, snapshot, settings, money, breakEven }) {
 ...
 return <>
  <div className="fin-trends-grid">
   <TrendChart title="Monthly margin trends" note="Gross margin is after COGS; operating margin is after COGS and all operating expenses, including selling; net margin is after all reported expenses and other income. Each is divided by that month’s revenue. Zero or negative revenue has no margin percentage." rows={rows} series={[{ key: 'grossMargin', label: 'Gross margin', color: '#635bce' }, { key: 'operatingMargin', label: 'Operating margin', color: '#168477' }, { key: 'netMargin', label: 'Net margin', color: '#bc743a' }]} format={percent}/>
