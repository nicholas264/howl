...
export default function FinanceTrends({ periods, months, snapshot, settings, money, breakEven }) {
 ...
 return <>
  <div className="fin-trends-grid">
   <TrendChart title="Monthly margin trends" note="Gross margin is after COGS; contribution margin is after COGS and the selected selling expenses; net margin is after all reported expenses and other income. Each is divided by that month’s revenue. Zero or negative revenue has no margin percentage." rows={rows} series={[{ key: 'grossMargin', label: 'Gross margin', color: '#635bce' }, { key: 'contributionMargin', label: 'Contribution margin', color: '#168477' }, { key: 'netMargin', label: 'Net margin', color: '#bc743a' }]} format={percent}/>
