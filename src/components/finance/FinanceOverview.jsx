...
export default function FinanceOverview({snapshot,periods,months,settings,ratios,pacing,costs,money,demo,onNavigate,scenarioContent,profitChart}){
 ...
 const monthlyBreakEven=Number.isFinite(ratios.breakEvenSales)&&months.length?ratios.breakEvenSales/months.length:null;
 const monthlyRevenue=months.length?ratios.revenue/months.length:null;
 ...
 <div className="fin-break-main"><div><strong>{money(monthlyBreakEven)}</strong><span>average monthly break-even revenue</span></div><p>{monthlyBreakEven==null?'Classify costs and establish a positive contribution margin to calculate break-even.':<>Your average revenue is <b>{money(monthlyRevenue)}</b> per month.</>}</p></div>
 <div className="fin-cost-equation"><div><span>{costs.method==='selling'?'Monthly operating budget':'Monthly fixed costs'}</span><strong>{money(Number.isFinite(ratios.fixedCosts)?ratios.fixedCosts/months.length:null)}</strong></div><span aria-hidden="true">÷</span><div><span>Contribution margin</span><strong>{pct(ratios.contributionMargin)}</strong></div><button onClick={()=>onNavigate('costs')}>{costs.unclassified.length?`Review ${costs.unclassified.length} accounts`:'Review cost assumptions'}</button></div>
 <p className="fin-footnote">{costs.method==='selling'?`Revenue less COGS and selling expenses. Break-even holds the remaining operating budget constant at its ${months.length}-month average and assumes the same COGS and selling-expense share of sales.`:`Based on the ${months.length}-month average and your cost classifications.`} Excludes other income and expenses, debt principal, and capital spending.</p>{scenarioContent}</section>
