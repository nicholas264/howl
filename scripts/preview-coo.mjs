...
const save=(kind,values)=>{state=applyCooCommand(state,{action:'save',kind,values},'Demo COO',now);return state[{cycle:'cycles',objective:'objectives',metric:'metrics',initiative:'initiatives',review:'reviews'}[kind]].at(-1);};
...
const configs=[
 {index:0,title:'Build a reliable production engine',owner:'Jordan',metric:'On-time production',baseline:80,target:95,unit:'%',direction:'increase',value:89,previous:85,initiative:'Standardize final assembly checks',status:'at-risk',blocker:'Two assembly stations need revised work instructions.'},
 {index:1,title:'Grow profitable customer demand',owner:'Alex',metric:'Qualified sales pipeline',baseline:200000,target:450000,unit:'USD',direction:'increase',value:465000,previous:385000,initiative:'Launch the autumn dealer program',status:'on-track',blocker:''},
 {index:2,title:'Keep critical parts available',owner:'Morgan',metric:'Supplier on-time delivery',baseline:75,target:95,unit:'%',direction:'increase',value:81,previous:88,initiative:'Qualify a second component supplier',status:'blocked',blocker:'Waiting on sample approval before the next purchase order.'},
];
for(const c of configs){const departmentId=state.departments[c.index].id;state.departments[c.index].owner=c.owner;
 ...
 const metric=save('metric',{title:c.metric,owner:c.owner,departmentId,cycleId:cycle.id,objectiveId:objective.id,kind:'kr',baseline:c.baseline,target:c.target,unit:c.unit,direction:c.direction,cadence:7,source:'Demo report',description:'Demonstration measure, entered manually.'});
 ...
 state=applyCooCommand(state,{action:'checkin',kind:'metric',id:metric.id,values:{date:today,value:c.value,blocker:c.blocker,note:'Demonstration weekly check-in'}},'Demo COO',now);
...
save('initiative',{title:'Approve the alternate supplier sample',owner:'Morgan',departmentId:state.departments[2].id,cycleId:cycle.id,dueDate:end,type:'action',reviewId:review.id,description:'Demo follow-up from the operating review.'});
await sql`INSERT INTO coo_workspace(id,data,updated_by) VALUES ('company',${JSON.stringify(state)}::jsonb,'demo')`;
const handler=createCooHandler({authorize:async(req,res,p)=>{const viewer=req.headers['x-coo-preview-role']==='viewer';if(viewer&&p==='analytics.write'){res.status(403).json({error:'Read-only access'});return null;}return {sql,userId:'Demo COO',permissions:viewer?['analytics.read']:['analytics.read','analytics.write']};}});
