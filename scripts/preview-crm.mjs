...
const previewPlugin={name:'isolated-crm-review',enforce:'pre',transformIndexHtml(html){return html.replace('<body>','<body><div style="background:#285840;color:white;padding:9px 16px;font:13px sans-serif;text-align:center">Local CRM preview · Sample data · Email sending is simulated</div>');},configureServer(server){server.middlewares.use(async(req,res,next)=>{
 const url=new URL(req.url,'http://localhost');if(url.pathname!=='/api/crm-email')return next();
 ...
 res.status=code=>{res.statusCode=code;return res;};res.json=body=>{res.setHeader('Content-Type','application/json');res.end(JSON.stringify(body));};await emailHandler(req,res);
});}};
const server=await createServer({plugins:[previewPlugin],server:{host:'127.0.0.1',port:5188,strictPort:true}});await server.listen();
console.log('Isolated CRM preview: http://127.0.0.1:5188/?tab=crm');
