...
if(process.argv.includes('--example')){
 ...
 await sql`INSERT INTO finance_connection(id,realm,tokens,expires_at,version,environment,connected_by) VALUES ('company',${encrypt('000',env)},${encrypt({access_token:'example-access',refresh_token:'example-refresh'},env)},now()+interval '1 hour',${nonce()},'sandbox','Local owner')`;
}
...
const server=await createServer({configFile:false,root:process.cwd(),envDir:emptyEnv,define:{'import.meta.env.VITE_AUTH_DISABLED':'"true"','import.meta.env.VITE_FINANCE_DEMO':'"true"'},plugins:[react(),{name:'finance-isolated-preview',transformIndexHtml(html){return html.replace('<body>','<body><div style="padding:8px 20px;background:#fff0cc;color:#624513;font:13px Helvetica;text-align:center">Financials local preview · All financial figures are illustrative · No live QuickBooks connection · Changes stay in a temporary database</div>');},configureServer(s){s.middlewares.use(async(req,res,next)=>{
 ...
 });}}],server:{host:'127.0.0.1',port:Number(process.env.FINANCE_PREVIEW_PORT||5195),strictPort:true}});
await server.listen();console.log(`Financials preview: http://127.0.0.1:${server.config.server.port}/?tab=finance`);
